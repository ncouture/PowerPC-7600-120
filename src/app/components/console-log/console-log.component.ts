...
@Component({
  ...
  template: `
    <div class="flex flex-col h-44 bg-black border-2 border-black mac-inset shadow-inner overflow-hidden">
      <!-- Terminal Header Bar -->
      <div class="bg-gray-900 border-b border-gray-700 px-3 py-1 flex justify-between items-center text-[10px] text-green-400 font-mono">
        <span class="flex items-center space-x-1.5">
          <span class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
          <span>PowerPC Assembly Log (Open Firmware v2.4)</span>
        </span>
        <div class="flex space-x-2">
          <button (click)="exportLog('txt')" class="hover:text-white underline cursor-pointer">Export TXT</button>
          <button (click)="exportLog('json')" class="hover:text-white underline cursor-pointer">Export JSON</button>
          <button (click)="clearLogs.emit()" class="hover:text-white underline cursor-pointer">Clear</button>
        </div>
      </div>
