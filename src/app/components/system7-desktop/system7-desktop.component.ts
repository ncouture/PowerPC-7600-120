...
@Component({
  ...
  template: `
    ...
    @if (isShutDown()) {
      <div
        class="fixed inset-0 bg-black z-[300] flex flex-col items-center justify-center text-white select-none font-mono space-y-6"
      >
        <div
          class="mac-window bg-white text-black p-6 border-2 border-black max-w-md w-full shadow-2xl text-center space-y-4"
        >
          <div class="text-3xl">💻</div>
          <h1 class="font-bold text-sm uppercase tracking-wider">
          ...
          <button
            (click)="restartSystem()"
            class="mac-button px-6 py-2 text-xs font-bold uppercase cursor-pointer"
          >
            Restart Macintosh
          </button>
        </div>
    ...
    @if (!isShutDown()) {
      ...
      >
        ...
        <div
          class="bg-white border-b-2 border-black px-3 py-0.5 flex justify-between items-center text-xs font-bold text-black z-50"
        >
          <!-- Left Menu Items -->
          <div class="flex space-x-4">
            ...
            <div class="relative">
              ...
              @if (activeMenu() === 'apple') {
                ...
                >
                  <div
                    (click)="openIcon('bithacks'); closeMenu(); $event.stopPropagation()"
                    class="px-3 py-1.5 hover:bg-black hover:text-white cursor-pointer border-b border-gray-200"
                  >
                    About PowerPC Bit-Hacks...
                  </div>
                  <div
                    (click)="openSoundControlPanel(); closeMenu(); $event.stopPropagation()"
                    class="px-3 py-1.5 hover:bg-black hover:text-white cursor-pointer border-b border-gray-200 flex justify-between items-center"
                  >
                    <span>Control Panels -> Sound...</span>
                    <span class="text-[10px] text-gray-500 font-normal">(⌘S)</span>
                  </div>
                  <div
                    (click)="setDesktopPattern('gray'); closeMenu(); $event.stopPropagation()"
                    class="px-3 py-1.5 hover:bg-black hover:text-white cursor-pointer"
                  >
                    Desktop Pattern: Classic Gray
                  </div>
                  <div
                    (click)="setDesktopPattern('platinum'); closeMenu(); $event.stopPropagation()"
                    class="px-3 py-1.5 hover:bg-black hover:text-white cursor-pointer"
                  >
                    Desktop Pattern: Platinum Grid
                  </div>
                  <div
                    (click)="setDesktopPattern('teal'); closeMenu(); $event.stopPropagation()"
                    class="px-3 py-1.5 hover:bg-black hover:text-white cursor-pointer border-b border-gray-200"
                  >
                    Desktop Pattern: Teal Tiles
                  </div>
                  <div
                    (click)="cleanUpDesktop(); closeMenu(); $event.stopPropagation()"
                    class="px-3 py-1.5 hover:bg-black hover:text-white cursor-pointer"
                  >
            ...
            <div class="relative">
              ...
              @if (activeMenu() === 'view') {
                ...
                >
                  ...
                  <div
                    (click)="resetIconPositions(); closeMenu(); $event.stopPropagation()"
                    class="px-3 py-1.5 hover:bg-black hover:text-white cursor-pointer"
                  >
            ...
            <div class="relative">
              ...
              @if (activeMenu() === 'sound') {
                ...
                >
                  ...
                  <div
                    (click)="soundService.toggleMute(); closeMenu(); $event.stopPropagation()"
                    class="px-3 py-1.5 hover:bg-black hover:text-white cursor-pointer flex justify-between items-center border-b border-gray-200"
                  >
                    <span>{{ soundService.isMuted() ? 'Unmute Audio' : 'Mute Audio' }}</span>
                    <span class="text-[10px] text-gray-500 font-normal">(⌘+M)</span>
                  </div>
                  <div class="px-3 py-1 text-[10px] text-gray-500 font-bold uppercase bg-gray-100">
                    Quick Volume Level
                  </div>
                  <div
                    (click)="setVolume(0); closeMenu(); $event.stopPropagation()"
                  ...
                  >
                    ...
                    @if (soundService.macVolumeLevel() === 0) {
                      <span>✓</span>
                    }
                  </div>
                  <div
                    (click)="setVolume(30); closeMenu(); $event.stopPropagation()"
                    class="px-3 py-1 hover:bg-black hover:text-white cursor-pointer flex justify-between items-center text-[11px]"
                  >
                    <span>Level 2 (Quiet - 30%)</span>
                    @if (soundService.macVolumeLevel() === 2) {
                      <span>✓</span>
                    }
                  </div>
                  <div
                    (click)="setVolume(60); closeMenu(); $event.stopPropagation()"
                    class="px-3 py-1 hover:bg-black hover:text-white cursor-pointer flex justify-between items-center text-[11px]"
                  >
                    <span>Level 4 (Medium - 60%)</span>
                    @if (soundService.macVolumeLevel() === 4) {
                      <span>✓</span>
                    }
                  </div>
                  <div
                    (click)="setVolume(100); closeMenu(); $event.stopPropagation()"
                    class="px-3 py-1 hover:bg-black hover:text-white cursor-pointer flex justify-between items-center text-[11px] border-b border-gray-200"
                  >
                    <span>Level 7 (Max - 100%)</span>
                    @if (soundService.macVolumeLevel() === 7) {
                      <span>✓</span>
                    }
                  </div>
                  <div class="px-3 py-1 text-[10px] text-gray-500 font-bold uppercase bg-gray-100">
                  ...
                  @for (theme of soundThemes; track theme) {
                    ...
                    >
                      ...
                      @if (soundService.soundTheme() === theme.id) {
                        <span>✓</span>
                      }
                    </div>
                  }
            ...
            <div class="relative">
              <span
                (click)="toggleMenu('special', $event)"
              ...
              @if (activeMenu() === 'special') {
                ...
                >
                  ...
                  <div
                    (click)="emptyTrash(); closeMenu(); $event.stopPropagation()"
                    class="px-3 py-1.5 hover:bg-black hover:text-white cursor-pointer border-b border-gray-200"
                  >
                    Empty Trash...
                  </div>
                  <div
                    (click)="restartSystem(); closeMenu(); $event.stopPropagation()"
                    class="px-3 py-1.5 hover:bg-black hover:text-white cursor-pointer border-b border-gray-200"
                  >
                    Restart
                  </div>
                  <div
                    (click)="shutdownSystem(); closeMenu(); $event.stopPropagation()"
                    class="px-3 py-1.5 hover:bg-black hover:text-white cursor-pointer"
                  >
                    Shut Down
                  </div>
          ...
          <div class="flex items-center space-x-2">
            ...
            <span
              (click)="isBitHacksOpen.set(true)"
              class="bg-black text-white px-1.5 py-0.5 text-[10px] font-mono cursor-pointer"
              >🖥️ Finder</span
            >
          </div>
        </div>
        ...
        <div class="flex-1 relative p-4 overflow-hidden">
          ...
          @for (icon of desktopIcons; track icon) {
            <div
            ...
            >
              <div
                [ngClass]="{ 'bg-black/20 p-1 border border-black': selectedIconId() === icon.id }"
              ...
              <span
                [ngClass]="{
                  'bg-black text-white': selectedIconId() === icon.id,
                  'bg-white/80 text-black border border-black': selectedIconId() !== icon.id,
                }"
          ...
          @if (isBitHacksOpen()) {
            <app-mac-window (closeWindow)="isBitHacksOpen.set(false)"></app-mac-window>
          }
            ...
            >
              <div class="mac-title-bar px-2 py-0.5 flex justify-between items-center">
                <span class="text-xs font-bold text-black font-mono"
                  >💽 Macintosh HD (64MB Available)</span
                >
                <button
                  (click)="isMacHdOpen.set(false)"
                  class="w-4 h-4 bg-white border border-black text-[10px] font-bold font-mono hover:bg-red-300"
                >
                  ×
                </button>
              </div>
              <div
                class="grid grid-cols-3 gap-3 p-3 bg-amber-50/40 border border-black text-center text-xs"
              >
                <div
                  (dblclick)="openIcon('bithacks')"
                  class="p-2 border border-black bg-white hover:bg-yellow-100 cursor-pointer space-y-1"
                >
          ...
          @if (isTrashOpen()) {
            ...
            >
              ...
              <div class="p-3 bg-amber-50/40 border border-black text-center text-xs space-y-3">
                ...
                <button
                  (click)="emptyTrash()"
                  class="mac-button px-4 py-1 text-xs font-bold cursor-pointer"
                >
                  Empty Trash
                </button>
              </div>
        ...
        <app-sound-control-panel [isOpen]="soundPanelOpen()" (close)="soundPanelOpen.set(false)">
    ...
    if (typeof localStorage !== 'undefined') {
      ...
      if (savedPositions) {
        try {
          ...
          // Ignore JSON parse errors for saved positions
        }
      }
    }
  }
