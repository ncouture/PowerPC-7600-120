...
@Component({
  ...
  template: `
    <div class="bg-gray-200 border-t-2 border-black px-4 py-1 text-[11px] flex justify-between items-center select-none font-mono">
      <span class="flex items-center space-x-2">
        <span>CPU: PowerPC 604e @ 200 MHz</span>
        <span class="text-gray-500">|</span>
        <span class="text-green-800 font-bold">AltiVec Unit: Active</span>
        <span class="text-gray-500">|</span>
        <span (click)="openSound.emit()" 
              [title]="soundService.isMuted() ? 'Audio Muted - Click to Open Sound Control Panel' : 'Volume Level ' + soundService.macVolumeLevel() + '/7 (Click for Sound Panel)'"
          ...
          <span>{{ soundService.isMuted() ? 'MUTED' : 'Sound (' + soundService.macVolumeLevel() + '/7)' }}</span>
        </span>
      </span>
