...
import { SoundEffectsService, SoundTheme, DacQuality } from '../../services/sound-effects.service';
      ...
      <div #panelContainer
        ...
        <div (mousedown)="startDrag($event)"
          ...
          <div class="flex items-center space-x-2 bg-white px-2 py-0.5 border border-black pointer-events-none">
            ...
            <span [id]="titleId" class="text-xs font-bold tracking-wider font-mono text-black">Sound Control Panel</span>
          </div>
          <div (click)="closeModal()" 
               title="Close Control Panel"
               class="w-4 h-4 bg-white border border-black flex items-center justify-center text-[10px] font-bold cursor-pointer hover:bg-red-300 text-black z-10">×</div>
        </div>
        ...
        <div class="p-4 pinstripes space-y-4 text-xs font-mono">
          ...
          <div class="bg-white border-2 border-black p-3 shadow-[3px_3px_0px_#000] space-y-3">
            <div class="flex items-center justify-between border-b border-black pb-1">
              <span class="font-bold text-black uppercase tracking-wider">Volume & Hardware Level</span>
            ...
            <div class="bg-black p-1.5 border border-black flex items-center space-x-1">
              ...
              <div *ngFor="let seg of vuSegments; let i = index" 
                   [ngClass]="i < activeVuCount() ? (i > 7 ? 'bg-red-500' : (i > 5 ? 'bg-yellow-400' : 'bg-green-400')) : 'bg-gray-800'"
          ...
          <div class="bg-white border-2 border-black p-3 shadow-[3px_3px_0px_#000] space-y-2">
            ...
            <div class="mac-inset bg-amber-50/40 p-1 max-h-32 overflow-y-auto space-y-1">
              <div *ngFor="let theme of soundThemes" 
                   ...
                   [ngClass]="{'bg-black text-white': soundService.soundTheme() === theme.id, 'hover:bg-gray-200 text-black': soundService.soundTheme() !== theme.id}"
                   class="px-2 py-1 flex items-center justify-between cursor-pointer font-bold text-[11px]">
                <span>{{ theme.label }}</span>
                <span [ngClass]="{'text-gray-400': soundService.soundTheme() === theme.id, 'text-gray-600': soundService.soundTheme() !== theme.id}" class="text-[9px] font-normal font-mono">
                  {{ theme.type }}
                </span>
              </div>
            </div>
          ...
          <div class="bg-white border-2 border-black p-3 shadow-[3px_3px_0px_#000] flex items-center justify-between">
            <div>
              <div class="font-bold text-black">DAC Audio Resampling:</div>
              <div class="text-[9px] text-gray-600">Simulate PowerPC 8-bit 22kHz vs 16-bit 44.1kHz</div>
            </div>
            <div class="flex items-center space-x-1">
              <button (click)="soundService.setDacResampling('8bit')" 
              ...
              <button (click)="soundService.setDacResampling('16bit')" 
          ...
          <div class="flex justify-between items-center pt-1">
            <button (click)="soundService.resetToDefaults(); soundService.playAlert()" 
