...
import {
  ...
  EntropySource,
  CardResult,
} from '../../services/bit-hacks.service';
    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
      ...
      <div class="md:col-span-5 space-y-4">
        ...
        <div class="bg-white border-2 border-black p-4 shadow-[3px_3px_0px_#000]">
          <h2 class="text-xs uppercase bg-black text-white px-2 py-1 font-bold mb-3 tracking-wider">
            1. Select ISO/IEC 7812 Issuer
          </h2>
        ...
        <div class="bg-white border-2 border-black p-4 shadow-[3px_3px_0px_#000]">
          <h2 class="text-xs uppercase bg-black text-white px-2 py-1 font-bold mb-3 tracking-wider">
            2. PowerPC Bit-Hacks Engine
          </h2>
          <div class="space-y-3 text-xs">
            <div>
              <label for="entropy-select" class="block font-bold mb-1">Bitwise Entropy Source:</label>
              <select id="entropy-select" [(ngModel)]="selectedEntropy" class="w-full mac-inset p-1 text-xs font-mono">
            ...
            <div>
              <label for="batch-input" class="block font-bold mb-1">Batch Generation Count:</label>
              <input
                id="batch-input"
                type="number"
                [(ngModel)]="batchCount"
                min="1"
                max="20"
                class="w-full mac-inset p-1 text-xs font-mono"
              />
            </div>
            <div class="flex items-center space-x-2 pt-1">
              <input type="checkbox" id="autoLuhn" [(ngModel)]="autoLuhn" class="accent-black" />
              <label for="autoLuhn" class="font-bold cursor-pointer"
                >Enforce Luhn Checksum Mod-10 BitMask</label
              >
            </div>
          </div>
        </div>
      ...
      <div class="md:col-span-7 flex flex-col space-y-4">
        ...
        <div
          class="bg-white border-2 border-black p-4 flex-1 flex flex-col shadow-[3px_3px_0px_#000]"
        >
          <div class="flex justify-between items-center mb-2">
            <h2 class="text-xs uppercase font-bold tracking-wide">
              Generated Valid Datastreams (Luhn Verified)
            </h2>
            <button
              (click)="clearResults()"
              class="mac-button px-2 py-0.5 text-[10px] cursor-pointer"
            >
              Clear Buffer
            </button>
          </div>
          ...
          <div
            class="mac-inset flex-1 p-3 font-mono text-xs overflow-y-auto min-h-[160px] max-h-[240px] bg-amber-50/30 space-y-2"
          >
            ...
            @for (card of generatedCards(); track card) {
              <div
              ...
              >
                <div class="space-y-0.5">
                  <div class="font-bold text-black flex items-center space-x-2">
                    <span>💳</span>
                    <span class="tracking-widest font-mono text-sm">{{
                      formatDisplay(card.cardNumber)
                    }}</span>
                  </div>
                  <div class="text-[10px] text-gray-600">
                    Issuer: <span class="uppercase font-bold text-black">{{ card.issuer }}</span> |
                    Length: {{ card.length }} | Mode: {{ card.mode }}
                  </div>
                </div>
                <div class="flex items-center space-x-2">
                  <span
                    class="px-2 py-0.5 bg-green-200 border border-green-600 text-[10px] font-bold text-green-900"
                    >VERIFIED VALID</span
                  >
                  <button
                    (click)="copyCard(card.cardNumber)"
                    class="mac-button px-2 py-1 text-[10px] cursor-pointer"
                  >
                    Copy
                  </button>
                </div>
...
export class CardGeneratorComponent {
  private bitHacksService = inject(BitHacksService);
  private soundEffects = inject(SoundEffectsService);
  ...
  generate(): void {
    ...
    this.logEvent.emit(
      `Initiating vector sweep for issuer: ${this.selectedIssuer.toUpperCase()} via ${this.selectedEntropy}...`,
    );

    this.bitHacksService
      .streamCardBatch(this.selectedIssuer, this.batchCount, this.selectedEntropy)
      .subscribe({
        next: (card) => {
          this.generatedCards.update((cards) => [card, ...cards]);
          this.logEvent.emit(
            `Generated [${card.cardNumber}] -> Luhn Checksum: ${card.checkDigit} [Valid: ${card.isValid}]`,
          );
        },
        complete: () => {
          ...
          this.logEvent.emit(
            `Batch execution completed successfully. ${this.batchCount} records verified.`,
          );
        },
        error: () => {
          this.isStreaming.set(false);
