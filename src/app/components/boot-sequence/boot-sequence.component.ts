...
@Component({
  ...
  template: `
    <div *ngIf="!isBootComplete()" 
      ...
      <div class="mac-window bg-white border-2 border-black p-6 w-full max-w-lg shadow-2xl space-y-5 relative z-10">
        
        <!-- Happy Mac CRT Header -->
        <div class="flex items-center space-x-4 border-b-2 border-black pb-4">
          <div class="w-14 h-14 bg-black text-white border-2 border-black flex flex-col items-center justify-center text-2xl font-bold font-mono shadow-md animate-pulse">
            <span>😃</span>
            <span class="text-[8px] tracking-tighter uppercase font-mono">System 7</span>
          </div>
          <div>
            <h1 class="text-sm font-bold uppercase tracking-wider text-black">Welcome to Macintosh</h1>
            <div class="text-[11px] text-gray-600 font-mono">Starting up PowerPC 604e @ 200 MHz...</div>
          </div>
        </div>
        ...
        <div class="space-y-2">
          ...
          <div class="mac-inset p-1 bg-amber-50/50 border-2 border-black h-7 flex items-center">
            <div [style.width.%]="progress()" 
              ...
              <div *ngIf="progress() > 10" class="w-1.5 h-full bg-white/40"></div>
            </div>
          </div>
        ...
        <div class="text-center pt-1 border-t border-gray-300">
          <button (click)="openEasterEgg(); $event.stopPropagation()" 
      ...
      <div class="w-full max-w-4xl bg-white border-2 border-black p-3.5 shadow-xl z-10">
        <div class="text-[10px] font-bold text-black uppercase mb-1.5 flex justify-between items-center border-b border-black pb-1">
        ...
        <div class="flex items-center justify-between flex-wrap gap-2 py-1">
          <div *ngFor="let ext of extensionIcons" 
               [ngClass]="{'bg-black text-white': ext.loaded, 'bg-gray-200 text-gray-400 opacity-40': !ext.loaded}"
    ...
    <div *ngIf="isEasterEggOpen()" 
         class="fixed inset-0 bg-black/80 z-[300] flex items-center justify-center p-4 select-none"
         role="dialog" 
         aria-modal="true">
      <div class="mac-window bg-white border-2 border-black p-5 w-full max-w-2xl shadow-2xl space-y-4 relative">
        <div class="flex justify-between items-center border-b-2 border-black pb-2">
          <div class="flex items-center space-x-2">
            <span class="text-lg"></span>
            <h2 class="font-bold text-xs uppercase tracking-wider text-black">Apple Macintosh IIci Engineering Dev Team - 1989</h2>
          </div>
          <button (click)="closeEasterEgg()" class="mac-button px-2 py-0.5 text-xs font-bold">× Close</button>
        </div>

        <div class="border-2 border-black bg-black p-1">
          <img src="assets/apple_iici_dev_team_1989.jpg" 
        ...
        <div class="text-[11px] font-mono bg-amber-50 p-2 border border-black">
          <b>Secret Easter Egg Discovered!</b> "To the team that made the Mac IIci and System 7 possible: Thank you for laying the bitwise foundations of desktop personal computing."
        </div>
...
export class BootSequenceComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private soundService: SoundEffectsService,
  ...
  shouldTriggerSadMac(): boolean {
    ...
    try {
      ...
      if (params.has('sadmac') && params.get('sadmac') !== '0' && params.get('sadmac') !== 'false') {
        return true;
      }
    } catch (e) {}
